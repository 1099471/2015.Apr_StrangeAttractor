...
void StrangeAttractorApp::setup()
{
    ...
    cout << "NUMBER OF PORTS: " << mMidiOut.getNumPorts() << endl;
    for (int i = 0; i < mMidiOut.getNumPorts(); i++)
    {
        cout << mMidiOut.getPortName(i) << endl;
    }
    mMidiOut.openPort(0);
}
    ...
    gl::pushMatrices();
    {
        ...
        gl::pushMatrices();
        {
            ...
            for(int i = 0; i < mArraySize; i++){
                ...
                //miditest
                mMidiOut.sendControlChange(1, 1, mMovers[0]->loc.x);
                mMidiOut.sendControlChange(1, 2, mMovers[0]->loc.y);
                mMidiOut.sendControlChange(1, 3, mMovers[0]->loc.z);
            }
        }
        gl::popMatrices();
    }
